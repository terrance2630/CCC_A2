...
#######
for doc in source_db.view('_all_docs', skip=begin_read_idx, limit=interval):
    ...
    if language == 'en' and sa4_code != None:
        ...
        for token in token_list:
            ...
            if token in lst_vehicle:
                ...
                dict_s2_tweets_vehicle[sa4_code] += 1

                if sentiment > 0 :
                    dict_s2_postive[sa4_code] += 1
                elif sentiment == 0:
                    dict_s2_neutral[sa4_code] += 1
                else:
                    dict_s2_negative[sa4_code] += 1
                    #skip current token
...
if rank == 0:
    ...
    for code in sa4_codes:
        s2_number_tweets.append(dict_s2_tweets_vehicle[code])
        s2_postive.append(dict_s2_postive[code])
        s2_negative.append(dict_s2_negative[code])
        s2_neutrual.append(dict_s2_neutral[code])

    s2_combined_dict = {'sa4_code': sa4_codes, '# of tweets about traffic': s2_number_tweets,
                        '# of positive tweets': s2_postive, '# of negative tweets': s2_negative,
                        '# of neutral tweets': s2_neutrual}

    df_s2 = pd.DataFrame(s2_combined_dict)

    with open(f'{str(rank)}-senario2_data.csv', 'w') as out_f:
